...
from src.schemas.photos import PhotoResponse, PhotoResponseRating
...
@router.post("/{photo_id}/{rating}", response_model=RatingInDBBase, dependencies=[Depends(allowed_operation_post)])
...
async def calculate_rating(photo_id: int, db: AsyncSession = Depends(get_db),
                           current_user: User = Depends(auth_service.get_current_user)):
    average_rate = await rating_repository.calculate_rating(photo_id, db)

    print('average_rating',average_rate)
...
@router.get("/images_rating", response_model=List[PhotoResponseRating])
async def show_images_by_rating(sort_dsc: bool, db: AsyncSession = Depends(get_db),
                                current_user: User = Depends(auth_service.get_current_user)):
    images_by_rating = await rating_repository.show_images_by_rating(sort_dsc, db)
